...
public class Circle {
    private double radius;//Instance variable to store the radios of circle

    public Circle(double radius) {
        this.radius = radius < 0 ? 0 : radius;// Constructor to initialize the radius,ensuring its non negative
    }

    public double getRadius() {
        return radius;//Greeter method to retrieve the value of the radios
    }

    public double getArea() {
        return (radius * radius * Math.PI);//Method to calculate and return the area of the circle
    }
}
